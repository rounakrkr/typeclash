/**
 * Word pool and random text generator for TypeClash.
 * Contains 1000 common English words (lowercase, no punctuation).
 */
...
export const wordPool = [
  ...
  'bear', 'finish', 'happy', 'hope', 'flower', 'clothes', 'strange', 'gone', 'jump', 'baby',
  ...
  'offer', 'segment', 'duck', 'instant', 'market', 'degree', 'populate', 'chick', 'dear', 'enemy',
  'reply', 'drink', 'occur', 'support', 'speech', 'nature', 'range', 'steam', 'motion', 'path',
  'liquid', 'log', 'meant', 'quotient', 'teeth', 'shell', 'neck', 'program', 'public', 'universe'
];

/**
 * Generates a string of random words from wordPool until the target character count is reached or exceeded.
 * Guarantees no consecutive identical words.
 *
 * @param {number} [charTarget=100] - Minimum character count target
 * @returns {{ text: string, charCount: number }} Generated text and character count
 */
export function generateRandomWords(charTarget = 100) {
  const target = Math.max(1, Number(charTarget) || 100);
  const selectedWords = [];
  let lastWord = '';
  let currentLength = 0;
  ...
  while (currentLength < target) {
    let candidate;
    do {
      candidate = wordPool[Math.floor(Math.random() * wordPool.length)];
    } while (candidate === lastWord && wordPool.length > 1);

    selectedWords.push(candidate);
    lastWord = candidate;

    currentLength = selectedWords.join(' ').length;
  }
